    ...
    <Fragment>
      <div className={`h-screen bg-indigo-950 flex  sticky inset-y-0  top-0 left-0  pt-10  flex-col border-r shadow-sm   ${open ? "w-52":"w-20"} duration-300`}> 
           <div className="flex-shrink-0 pl-2 pr-1">
            {open ?  <a href="#" title="" className="flex">
              <img className="w-auto h-10" src={logo} alt="" />
              </a>
              :
              <h1 className='flex flex-wrap text-white font-extrabold uppercase'>Nasco Tech</h1>
              }
            </div>
          <div className='border-b border-slate-600 mx-1 pb-8'></div>
           ...
           ${!open && "rotate-180 "} duration-300 `} onClick={()=>setOpen(!open)}/>
            ...
            {SideBarItems.map((item,index)=>(
              <>
              <li key={index} className={`flex item-center gap-x-4 mt-5  text-sm font-medium 
              ${selected===index ? "text-blue-500 bg-gradient-to-r from-gray-500 border-l-4 border-indigo-500 ":"order-transparent  hover:bg-gradient-to-r from-gray-700   hover:border-l-4  hover:border-indigo-500"}
              duration-100 ${item && item.spacing && "mt-32"}`}
              onClick={() => handleSelected(index)}
              >
                  <Link to={item.path} className="flex item-center cursor-pointer  gap-x-4  p-2  pl-3 w-full py-2 text-gray-400 hover:text-gray-200">
